...
def process_dataset(df):
    ...
    drop_cols = [
        ...
        "gdp_per_capita_growth_annualized",
    ]
    ...
    missing_threshold = (
        50
        # Bạn có thể thay đổi giá trị này khi gọi hàm hoặc truyền tham số
    )
    ...
    if missing_after_interp > 0:  # Tắt Fill KNN
        ...
        if (
            not knn_fill_stats_df.empty
            and "fill_ratio_pct" in knn_fill_stats_df.columns
        ):
            ...
            print(
                "\nKhông có quốc gia nào được fill bằng KNN hoặc không có dữ liệu fill_ratio_pct."
            )
    print("\n=== KẾT QUẢ CUỐI CÙNG ===")
...
df = pd.read_csv(
    r"C:\Users\VAQ\OneDrive\Desktop\RP_2 ngươi bạn\C3_Menthology\01_machine learning method\data_csv\raw_data_3.csv"
)
